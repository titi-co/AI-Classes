...
class ExactInference(object):
    ...
    def __init__(self, numRows, numCols):
        # ONLY USED BY GRADER.PY in case problem 3 has not been completed
    ...
    def observe(self, agentX, agentY, observedDist):
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                priori = self.belief.getProb(row, col)
                x = util.colToX(col)
                y = util.rowToY(row)
                estimador = util.pdf(math.sqrt(
                    math.pow((agentX - x), 2) + math.pow((agentY - y), 2)), Const.SONAR_STD, observedDist)
                n = priori * estimador
                self.belief.setProb(row, col, n)
    ...
    def elapseTime(self):
        if self.skipElapse:
            return
        _belief = util.Belief(self.belief.getNumRows(),
                              self.belief.getNumCols(), 0)
        for (prevTile, nextTile) in self.transProb:
            p = self.belief.getProb(prevTile[0], prevTile[1])
            t = self.transProb[(prevTile, nextTile)]
            d = p * t
            _belief.addProb(nextTile[0], nextTile[1], d)
        _belief.normalize()
        self.belief = _belief
